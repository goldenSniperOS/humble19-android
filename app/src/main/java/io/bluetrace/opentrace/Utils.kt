...
import io.bluetrace.opentrace.services.BluetoothMonitoringService.Companion.PENDING_UPLOAD_RECORDS
...
object Utils {
    ...
    fun scheduleUploadRecords(context: Context, intervalMillis: Long) {
        ...
        nextIntent.putExtra(
            BluetoothMonitoringService.COMMAND_KEY,
            BluetoothMonitoringService.Command.ACTION_UPLOAD.index
        )

        Scheduler.scheduleRepeatingServiceIntent(
            PENDING_UPLOAD_RECORDS,
            context,
            nextIntent,
            intervalMillis
        )
